...
def HData(path):
    ...
    for i, plate_img in enumerate(plate_imgs):
        ...
        chars = pytesseract.image_to_string(img_result, lang='kor+eng', config='--oem 1 --psm 7')

        result_chars = ''
        has_digit = False
        for c in chars:
            if ord('a') <= ord(c) <= ord('힣') or c.isdigit():
                if c.isdigit():
                    has_digit = True
                result_chars += c
